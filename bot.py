...
"""
ULTIMATE BOT MANAGER (v11.6 - Robust Approval Logic)
Fixes Implemented:
1. RE-ARCHITECTED: 'LINK_MAP' now stores {user_id, batch_id}. 
   Bot no longer relies on 'Topic ID' to identify the user. This fixes the approval failure.
2. IMPROVED COMMANDS: /demo & /per now extract User ID directly from the stored Link data.
3. FIXED: Handled cases where user is already approved or link is expired.

Workflow:
1. User requests -> Bot saves {link: {user, batch}} -> Sends Link to Admin.
2. Admin uses /demo <link>.
3. Bot looks up Link -> Finds User -> Approves Request -> Sends DM.
"""# -*- coding: utf-8 -*-

"""
ULTIMATE BOT MANAGER (v11.8 - Admin Permission Fix)
Fixes Implemented:
1. FIXED: 'is_admin' function now ignores Type Mismatches (String vs Int).
   This solves the issue where Admins added via command were being ignored.
2. IMPROVED: Data Loading ensures IDs are handled correctly.

Features:
- Owner AND Admins can use /demo & /per.
- Robust Link Approval system.
"""
...
try:
    ...
    def _start_keepalive():
        ...
        app = Flask(__name__)
        @app.route('/')
        def index(): return "Bot Running - v11.8 Admin Fix", 200
...
async def cmd_approve_demo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update.effective_user.id): 
        ...
        # await update.message.reply_text("⚠️ Authorization Failed. Ask Owner to re-add you via /addadmin.")
        return
    ...
    try:
        ...
        # User Notification
        batch_name = DB["ALL_CHATS"].get(batch_id) or "Premium Channel"
        try: 
            await context.bot.send_message(
                target_uid, 
...
try:
    ...
    def _start_keepalive():
        ...
        app = Flask(__name__)
        @app.route('/')
        def index(): return "Bot Running - v11.6 Robust", 200
...
DB = {
    ...
    "USER_TOPICS": {}, 
    "PENDING_REQUESTS": {},
    "LINK_MAP": {}       # NOW STORES: invite_link -> {"u": user_id, "b": batch_id}
}
...
def main():
    ...
    print("Bot v11.6 Robust Approval Started...")
    app.run_polling()
